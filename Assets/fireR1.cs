...
public class fireR1 : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (test)
        {
            if (x == 0)
            {
                ...
                transform.Translate(Vector3.forward * 70, Space.Self);
                transform.localScale += new Vector3(0, 0, 100);
    ...
    void OnTriggerEnter(Collider other)
    {
        if (Fighter == AcadamyTIE || Fighter == ObsidianTIE)
        {
            ...
            {
                Debug.Log("Object hit R1");
                if (other.GetComponent<Rookie_T65>().rangeToFighter > 1)
                {
                    other.GetComponent<Rookie_T65>().rangeToFighter = 1;
                }
            }
        }
        ...
        if (Fighter == T65)
        {
            ...
            {
                Debug.Log("Object hit R1");
                if (other.GetComponent<TIE2>().rangeToFighter > 1)
                {
                    other.GetComponent<TIE2>().rangeToFighter = 1;
                }
            }
            ...
            {
                Debug.Log("Object hit R1");
                if (other.GetComponent<TIE1>().rangeToFighter > 1)
                {
                    other.GetComponent<TIE1>().rangeToFighter = 1;
                }
            }
        }
    }
